...
function App() {
    ...
    const getThemeStyles = () => {
        if (selectedDepartment === 'KIDS') return { bg: 'bg-pink-50', header: 'bg-pink-400 text-white shadow', text: 'text-pink-900', card: 'bg-white border-2 border-pink-200 p-6 rounded-2xl' };
        if (selectedDepartment === 'ADULT') return { bg: 'bg-slate-900', header: 'bg-slate-800 text-white shadow border-b-2 border-blue-500', text: 'text-white', card: 'bg-slate-800 border border-slate-700 p-6 rounded-xl text-white' };
        if (selectedDepartment === 'ORGAN') return { bg: 'bg-stone-100', header: 'bg-stone-800 text-emerald-400 shadow', text: 'text-stone-800', card: 'bg-stone-50 border border-emerald-300 p-6 rounded-lg' };
        return { bg: 'bg-slate-50', header: 'bg-white text-slate-900 shadow border-b-4 border-blue-600', text: 'text-slate-900', card: 'bg-white border-l-4 border-blue-500 p-6 shadow' };
    };
            ...
            <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4">
                <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-2xl border-t-8 border-blue-600">
                    ...
                    <form onSubmit={mockAuthenticate} className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-slate-600">Session Identity</label>
        ...
        <div className={`min-h-screen ${currentTheme.bg} transition-all duration-300 flex flex-col`}>
            <header className={`px-6 py-4 flex items-center justify-between ${currentTheme.header}`}>
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-lg font-black tracking-tight">Multi-Specialty App</h1>
                        <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">{userSession.name} ({userSession.role})</p>
                    </div>
                </div>
                <div className="flex bg-black bg-opacity-10 p-1 rounded-xl gap-1">
